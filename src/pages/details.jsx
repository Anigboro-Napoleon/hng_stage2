  ...
  return (
    <>
      <Flex justify={"center"} pos={"relative"}>
        ...
        >
          <Img src={`https://image.tmdb.org/t/p/w500/${backdrop_path}`} w={{ base: 'full', lg: '600px' }} borderRadius={'30px'} px={{ base: '15px', lg: 0 }} />
          <Box mt='15px' pl='20px'>
            <HStack align={{ base: 'start', lg: 'center' }} flexDir={{ base: 'column', lg: 'row' }}>
              <Text data-testid='movie-title' color='#000' fontWeight={600}>{title}</Text>
              <Text fontWeight={700} color='#000' display={{ base: 'none', lg: 'block' }}>.</Text>
              <Text data-testid='movie-release-date' color='#000' fontWeight={600}>{releaseDate}</Text>
              <Text fontWeight={700} color='#000' display={{ base: 'none', lg: 'block' }}>.</Text>
              <Text data-testid='movie-runtime' color='#000' fontWeight={600}>{`${runtime} mins`}</Text>
            </HStack>
            <Text data-testid='movie-overview' color='#000' fontSize={'1rem'} maxW={'600px'}>{overview}</Text>
          </Box>
        </Box>
      </Flex>
    </>
